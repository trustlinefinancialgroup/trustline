  ...
  return (
    <li className="px-5 py-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <p className="flex flex-wrap items-center gap-2 text-[15px] font-semibold text-navy-900">
            ...
            <span
              className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ${
                doc.required ? "bg-navy-100 text-navy-700" : "bg-gray-100 text-gray-500"
              }`}
            >
              {doc.required ? labels.required : labels.optional}
            </span>
          </p>
          <p className="mt-1 text-[13px] leading-relaxed text-gray-500">{doc.hint}</p>
          {doc.uploaded && (
            <p className="mt-1.5 flex flex-wrap items-center gap-2 text-xs text-gray-500">
              <span className="font-medium text-navy-800">{doc.uploaded.fileName}</span>
        ...
        {doc.uploaded && (
          <form action={deleteApplicationDocumentAction}>
            ...
            <button className="rounded-full border border-gray-200 px-3 py-1 text-xs font-semibold text-gray-600 transition hover:border-red-300 hover:text-red-600">
              {labels.remove}
            </button>
          </form>
        )}
      ...
      <form action={action} className="mt-3">
        ...
        <button
          disabled={pending}
          className="mt-2 rounded-full bg-navy-800 px-5 py-2 text-xs font-semibold text-white transition hover:bg-navy-700 disabled:opacity-60"
        >
